...
- `pathlon/` — the Claude Code plugin, and the canonical source for all methodology:
  ...
  - `agents/` — 6 agents (orchestrator + 5 specialists), each with a Definition of Done and a Done report it ends every run with. `commands/` — 7 slash commands (`/pathlon:kickoff`, `/pathlon:route`, `/pathlon:transition`, …). The `start` skill (`/pathlon:start`) is the front door: intake and routing.
  - `hooks/` — `context.mjs` via `hooks.json`: SessionStart injects the project's state, UserPromptSubmit adds a phase hint, PreCompact and SessionEnd record the session and a `usage` record (which Pathlon skills, agents and commands ran; no prompt text). Silent outside a Pathlon project. Test with `node --test pathlon/hooks/context.test.mjs`. SubagentStop prompt hooks check each specialist's Done report against its Definition of Done; they're generated from the agent files — after editing a Definition of Done, run `node pathlon/hooks/build-dod-checks.mjs` (tests fail if you forget). `.mcp.json` — runs the local Pathlon MCP (`server/index.mjs`) over stdio.
  ...
  - `tests/lint.test.mjs` — static checks on skill and agent frontmatter, descriptions, and retired names.
  - `server/report.mjs` — the usage report (also the `usage_report` MCP tool): usage, first-try Definition of Done rates, unused skills and gap notes across every registered project. Run `node pathlon/server/report.mjs`.
  - `evals/` — `claude plugin eval ./pathlon --judge-model sonnet --no-publish --trust-plugin` (about $9 of API-equivalent usage for the full suite; on a Max login it draws on plan usage). Results in `evals/results/` (gitignored).
  - Validate with `claude plugin validate ./pathlon`; try it with `claude --plugin-dir ./pathlon`. All tests: `node --test pathlon/server/*.test.mjs pathlon/hooks/*.test.mjs pathlon/tests/*.test.mjs`.
