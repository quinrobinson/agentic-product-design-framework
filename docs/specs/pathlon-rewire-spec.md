...
- **R5b suite written Sept 25, 2026:** `pathlon/evals/` has 10 cases in the format Claude Code 2.1.282 documents.
  - **Cases:** five skill-routing cases, each graded on outcome with a skill-fired indicator (research-synthesis, problem-framing, ux-copy-writing, motion, phase-handoff); delegation to `pathlon:researcher`; exactly one clarifying question for an ambiguous request; and three should-not-fire cases (a Python bug, a React component change, a git question).
  - **Mocks:** all 10 Pathlon MCP tools, as "no project in this folder".
  - **Checked by the runner:** it loaded all cases and mocks, then stopped at authentication at $0.
  - **To run:** Quin logs the CLI in (`claude`, then `/login`), then run a pilot (`--runs 1`), confirm the cost, then the full suite with `--judge-model sonnet`.
